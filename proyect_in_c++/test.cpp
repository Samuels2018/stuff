...
void initialMemory () {
	...
	int ii, jj, tamCache = 512, lineTam = 16, rep;

	rep = algorithm.line/2;
...
int main (int argc, char **argv) {
	...
	for (ii = 0; ii < algorithm.line; ii++) {
		for (jj = ii+1; jj <algorithm.line; jj++) {
			if (algorithm.block[ii] == algorithm.block[jj]) {
				algorithm.reply[ii] = algorithm.block[ii];
			}
		}
	}
...
/*[i]++ incrementa el valor del numero almacenado */